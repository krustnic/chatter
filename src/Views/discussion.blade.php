	                		...
	                		<li data-id="{{ $post->id }}">
		                		<span class="chatter_posts">
			                		...
			                		<div class="chatter_avatar">
					        			...
					        			@else
					        				<span class="chatter_avatar_circle" style="background-color:#<?= \DevDojo\Chatter\Helpers\Chatter::stringToColorCode($post->user->email) ?>">
					        		...
					        		<div class="chatter_middle">
					        			<span class="chatter_middle_details"><a href="{{ \DevDojo\Chatter\Helpers\Chatter::getUserLink($post->user) }}">{{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</a> <span class="ago chatter_middle_details">{{ \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->diffForHumans() }}</span></span>
	            ...
	            @if(!Auth::guest())
	            	...
	            	<div id="new_response">
	            		...
	            		<div class="chatter_avatar">
		        			...
		        			@else
		        				<span class="chatter_avatar_circle" style="background-color:#<?= \DevDojo\Chatter\Helpers\Chatter::stringToColorCode(Auth::user()->email) ?>">
